...
/*
Funcion que muestra los errores del SingUp
*/
function mostrarErrores(errors){
	//Remueve los hijso del div de errores
	var cell = document.getElementById("divErroresSingUp");
	if ( cell.hasChildNodes() ){
		while ( cell.childNodes.length >= 1 ){
			cell.removeChild( cell.firstChild );
		}
...
/*
Funcion que crea un popUP que muestra el login para el usuario
*/
function mostrarLogin(){
	var cell = document.getElementById("panelLogin");
	if ( !cell.hasChildNodes() ){
		var divContent = $("<div></div>").attr("class","modal-content");
		var divBody	= $("<div></div>").attr("class","modal-body");
		var titulo = $("<h1></h1>").text("Login");
		titulo.attr("class","modal-title");
		var divHeader = $("<div></div>").attr("class","modal-header");
		divHeader.append(titulo);
		...
		formulario.append(divEmail);
		var divPassword = $("<div></div>").attr("class","form-group");
		var lblPassword = $("<label></label>").attr("for","password");
		...
		formulario.append(divPassword);
		var btnSubmit = $("<button></button>").attr("type","submit");
		btnSubmit.attr("class","btn btn-primary");
		btnSubmit.text("Entrar");
		formulario.append(btnSubmit);
		var btnFacebook = $("<button></button>").attr("class","btn btn-block btn-social btn-facebook");
		var span = $("<span></span>").attr("class","fa fa-facebook");
		btnFacebook.append(span);
		btnFacebook.append("Ingrese con Facebook");


		divBody.append(formulario);
		divBody.append(btnFacebook);
		divContent.append(divHeader);
		divContent.append(divBody);
		$(panelLogin).append(divContent);
	}
	$(panelLogin).dialog({
		closeText: 'X'
	});
